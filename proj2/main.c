...
char * opcode (unsigned int cur_buf, char * assembly_string, int registers_p[], unsigned char data_memory[]);
char * rType (unsigned int cur_buf, char * assembly_string, int registers_p[], unsigned char data_memory[]);
char * iType (unsigned int cur_buf, char * assembly_string, int registers_p[],unsigned char data_memory[]);
...
int main(int argc, char** argv){
	...
	long n_times = strtol(argv[1], NULL, 10);
	int registers[33];
	...
	for (long i = 0; i< n_times; i++) {
                opcode(current_instruction, assembly_string, registers, data_memory);
...
char * opcode (unsigned int cur_buf, char * assembly_string, int registers[], unsigned char data_memory[]) {
	...
	switch (opcode_val) {
		case 0:
			...
			assembly_string = rType(cur_buf, assembly_string, registers, data_memory);
			break;
		...
		default:
			assembly_string = iType(cur_buf, assembly_string, registers, data_memory);
			break;
	}
...
char * rType (unsigned int cur_buf, char * assembly_string, int registers[], unsigned char data_memory[]) {
...
char * iType (unsigned int cur_buf, char * assembly_string, int registers[], unsigned char data_memory[]) {
	...
	op = (cur_buf >> 26) & 0x2ffffff;
		...
		case 0x23: //lw
			...
			for (int i=0; i<4; i++) {
				lw_val = lw_val << 8; 
				lw_val = lw_val | (int)data_memory[registers[rs]+signed_imm + i-0x10000000];
		...
		case 0x2b: //sw
			...
			for (int i=0; i<4; i++) {
				...
				data_memory[registers[rs]+signed_imm + i-0x10000000] = temp;
